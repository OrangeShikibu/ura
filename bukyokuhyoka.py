...
df_wos.pivot_table(values='UT (Unique WOS ID)',columns=['Document Type'],index=['Publication Year'],aggfunc=len)
...
# 金研ではない所属機関情報のものを除く
if bukyokumei == 'IMR':
    notIMR = ['Tohoku Univ, Inst Mat Res, Inst Fluid Sci & Adv, Sendai, Miyagi 9808577, Japan']
    df_wos_bukyoku = df_wos_bukyoku[~df_wos_bukyoku['Addresses'].str.contains(notIMR[0])]
...
df_wos_imr.pivot_table(values="UT (Unique WOS ID)", columns=["Document Type"], index=["Publication Year"], aggfunc=len)
